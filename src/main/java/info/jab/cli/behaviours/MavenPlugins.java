    ...
    public void execute(String parameter) {
        if ("flatten".equals(parameter)) {
            try {
                ...
                String flattenPluginXmlPath = "templates/maven-plugins-template/maven-plugins-flatten.xml";
                ...
                // 1. Read and parse maven-plugins-flatten.xml
                File flattenPluginFile = new File(flattenPluginXmlPath);
                if (!flattenPluginFile.exists() || !flattenPluginFile.isFile()) {
                    System.err.println("Error: Flatten plugin XML file not found or is not a file: " + flattenPluginFile.getAbsolutePath());
                    return;
                }
                Document flattenDoc = db.parse(flattenPluginFile);
                ...
                pluginsElement.appendChild(importedPluginNode); // Appends as the last child
