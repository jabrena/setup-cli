...
class DependabotTest {
    ...
    void shouldCallCopyFilesWithCorrectParameters() {
        ...
        // Then
        verify(mockCopyFiles).copyClasspathFolder(eq("dependabot/"), eq(expectedGithubPath));
    }
    ...
    void shouldResolveCorrectGithubPath() {
        ...
        // Then
        verify(mockCopyFiles).copyClasspathFolder(eq("dependabot/"), any(Path.class));

        // Verify the path resolution logic by checking the actual path passed
        Path currentPath = Paths.get(System.getProperty("user.dir"));
        Path expectedPath = currentPath.resolve(".github");
        verify(mockCopyFiles).copyClasspathFolder(eq("dependabot/"), eq(expectedPath));
    }
}
